...
def get_pie_chart(entered_site):
    # A new df to contain all data relating to the that specific site
    filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site]
    # Count all successful and failled launches count 
    success_count = filtered_df[filtered_df['class'] == 1].shape[0]
    failed_count = filtered_df[filtered_df['class'] == 0].shape[0]
    
    All = spacex_df[['Launch Site', 'class']]
    All = All[All['class'] == 1].groupby(by = 'Launch Site')[['class']].sum().reset_index()

    if entered_site == 'ALL':
        fig = px.pie(data_frame = spacex_df, 
                     values = (All['class']), 
                     names = list(launch_sites), 
        ...
        fig = px.pie(data_frame = filtered_df, 
                     ...
                     names=['Succesful Landing', 'Failed Landing'], 
...
def get_scatter_chart(entered_site, payload_range):
    # A new df to contain all data relating to the that specific site and where the PM(kg) is in between or equals the payload range slider points
    filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site]
